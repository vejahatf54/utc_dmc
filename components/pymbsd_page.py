    ...
    return dmc.Container([
        ...
        dmc.Stack([
            dmc.Center([
                dmc.Stack([
                    dmc.Group([
                        dmc.Title("PyMBSd Service Management", order=2, ta="center"),
                        dmc.ActionIcon(
                    ...
                    dmc.Grid([
                        dmc.GridCol([
                            dmc.Stack([
                                ...
                                dmc.List([
                                    dmc.ListItem("Administrator privileges required"),
                                    dmc.ListItem("Access to UNC package paths"),
                                    dmc.ListItem("WinSW service wrapper support"),
                                    dmc.ListItem("Valid PyMBSd packages in configured location")
                                ], size="sm")
                            ])
                        ], span=6),
                        dmc.GridCol([
                            dmc.Stack([
                                ...
                                dmc.List([
                                    dmc.ListItem("Click 'Fetch Services' to load available packages"),
                                    dmc.ListItem("Select services using checkboxes"),
                                    dmc.ListItem("Use Install/Start/Stop/Uninstall buttons for operations"),
                                    dmc.ListItem("Monitor status and logs in the right panel")
                                ], size="sm")
                            ])
                        ], span=6)
                    ])
            ...
            dmc.Grid([
                ...
                dmc.GridCol([
                    dmc.Paper([
                        dmc.Group([
                            dmc.Title("Available Services", order=4, c="dimmed"),
                            dmc.Button(
                                "Refresh List",
                                id="pymbsd-refresh-btn",
                                leftSection=BootstrapIcon(icon="arrow-clockwise", width=16, height=16),
                                variant="outline",
                                color="blue",
                                size="sm"
                            ),
                ...
                dmc.Paper([
                    ...
                    dmc.Stack([
                        dmc.Button(
                            ...
                            leftSection=BootstrapIcon(icon="download", width=16, height=16),
                            color="green",
                            size="md",
                            disabled=True,
                            fullWidth=True
                        ...
                        dmc.Button(
                            "Start",
                            id="pymbsd-start-btn",
                            leftSection=BootstrapIcon(icon="play-fill", width=16, height=16),
                            color="blue",
                            size="md",
                            disabled=True,
                            fullWidth=True
                        ...
                        dmc.Button(
                            "Stop",
                            id="pymbsd-stop-btn",
                            leftSection=BootstrapIcon(icon="stop-fill", width=16, height=16),
                            color="orange",
                            size="md",
                            disabled=True,
                            fullWidth=True
                        ...
                        dmc.Button(
                            ...
                            leftSection=BootstrapIcon(icon="trash", width=16, height=16),
                            color="red",
                            size="md",
                            disabled=True,
                            fullWidth=True
...
def create_service_card(service_info, index):
    ...
    icon_class = "service-status-loading" if status in ["starting", "stopping"] else ""
...
def refresh_service_list(n_clicks):
    ...
    try:
        # Show loading
        services = PyMBSdService.fetch_service_packages()
        
        service_cards = []
        for i, service in enumerate(services):
            service_cards.append(create_service_card(service, i))
        ...
        if "not accessible" in error_text:
            ...
            icon = BootstrapIcon(icon="wifi-off", width=20, height=20, className="text-danger")
        elif "not configured" in error_text:
            title = "Configuration Error"
            icon = BootstrapIcon(icon="gear", width=20, height=20, className="text-danger")
        else:
            title = "Error Loading Services"
            icon = BootstrapIcon(icon="exclamation-triangle", width=20, height=20, className="text-danger")
...
def enable_status_updates(service_list):
    """Enable status updates when services are loaded"""
...
def update_service_status(n_intervals, service_data):
    """Update service status icons periodically"""
    if not service_data:
...
@callback(
    Output({"type": "pymbsd-service-checkbox", "index": ALL}, "checked"),
    Input("pymbsd-select-all", "checked"),
    State({"type": "pymbsd-service-checkbox", "index": ALL}, "checked"),
    prevent_initial_call=True
)
def toggle_select_all(select_all, current_selections):
    """Handle select all checkbox"""
    if current_selections:
        return [select_all] * len(current_selections)
    return []
...
def update_button_states(checked_services):
    """Enable/disable action buttons based on service selection"""
    has_selection = any(checked_services) if checked_services else False
...
def handle_service_actions(install_clicks, start_clicks, stop_clicks, uninstall_clicks,
                          checked_services, service_data, start_after_install, auto_mode):
    """Handle service management actions"""
    if not ctx.triggered or not any(checked_services):
        return []
    
    # Get selected services
    selected_indices = [i for i, checked in enumerate(checked_services) if checked]
    selected_services = [service_data[i] for i in selected_indices if i < len(service_data)]
    ...
    try:
        if "install" in triggered_id:
            result = PyMBSdService.install_services(selected_services, start_after_install, auto_mode)
