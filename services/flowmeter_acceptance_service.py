...
class FlowmeterAcceptanceService:
    ...
    def run_analysis(self, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            self.logger.info("Starting simplified flowmeter acceptance analysis")
            ...
            self.logger.info(f"Loaded {len(self.tags_df)} meter configurations")
            
            # Validate Tags.in format
            required_columns = ['SCADATagID_DIG', 'SCADATagID_ANL', 'MBSTagID', 'Reference_Meter']
            missing_columns = [col for col in required_columns if col not in self.tags_df.columns]
            if missing_columns:
                raise ValueError(f"Tags file is missing required columns: {missing_columns}. Please use Tags.in format.")
            ...
            plots = self.create_analysis_plots({'template': self.current_theme})
    ...
    def _run_meter_tests(self, meter_name: str, digital_tag: str, analog_tag: str, 
        ...
        try:
            ...
            rtu_data_available = self._check_rtu_data(digital_tag, analog_tag, rtu_file, time_start, time_end)
            review_data_available = self._check_review_data(meter_name, ref_tag, review_file, time_start, time_end)
            ...
            results['overall_status'] = 'pass' if all(status == 'pass' for status in all_tests) else 'fail'
    ...
    def create_analysis_plots(self, theme_data: Optional[Dict[str, Any]] = None) -> Dict[str, go.Figure]:
        ...
        if not self.test_results:
            ...
            plots['status'] = self._create_placeholder_plot(plotly_template, "Run Analysis to See Results")
            return plots
        
        # Plot 1: Overall Test Results
        meters = list(self.test_results.keys())
        statuses = [self.test_results[meter]['overall_status'] for meter in meters]
        
        colors = ['green' if status == 'pass' else 'red' for status in statuses]
        
        fig1 = go.Figure(data=[
            go.Bar(x=meters, y=[1]*len(meters), marker_color=colors, name='Test Results')
        ])
        fig1.update_layout(
        ...
        fig2 = go.Figure(data=[
            go.Bar(name='Passed', x=categories, y=pass_counts, marker_color='green'),
            go.Bar(name='Failed', x=categories, y=fail_counts, marker_color='red')
        ])
        fig2.update_layout(
        ...
        try:
            ...
            for index, row in self.tags_df.iterrows():
                ...
                if digital_tag:
                    ...
                    self._export_rtu_tag_data(rtu_file, digital_tag, dig_csv_file, time_start, time_end)
                    exported_files.append(dig_csv_file)
                
                # Export RTU data for analog tag
                if analog_tag:
                    anl_csv_file = os.path.join(data_dir, "SCADATagID_ANL.csv")
                    self._export_rtu_tag_data(rtu_file, analog_tag, anl_csv_file, time_start, time_end)
                    exported_files.append(anl_csv_file)
                
                # Export Review data for MBS tag
                if meter_name:
                    mbs_csv_file = os.path.join(data_dir, "MBSTagID.csv")
                    self._export_review_tag_data(review_file, meter_name, mbs_csv_file, time_start, time_end)
                    exported_files.append(mbs_csv_file)
                
                # Export Review data for Reference tag
                if ref_tag:
                    ref_csv_file = os.path.join(data_dir, "Reference_Meter.csv")
                    self._export_review_tag_data(review_file, ref_tag, ref_csv_file, time_start, time_end)
                    exported_files.append(ref_csv_file)
            
            return {
                'success': True,
                'exported_files': list(set(exported_files)),  # Remove duplicates
    ...
    def _export_rtu_tag_data(self, rtu_file: str, tag_name: str, output_file: str, 
        ...
        try:
            ...
            try:
                # Use RTU service to export CSV data for the specific tag
                ...
                self.logger.info(f"Successfully exported RTU tag {tag_name} to {output_file}")
            finally:
    ...
    def _export_review_tag_data(self, review_file: str, tag_name: str, output_file: str,
        ...
        try:
            self.logger.info(f"Exporting Review tag: {tag_name} to {output_file}")
            ...
            try:
                ...
                source_file = os.path.join(review_folder, os.path.basename(output_file))
                if os.path.exists(source_file):
                    if source_file != output_file:
                        shutil.move(source_file, output_file)
                    self.logger.info(f"Successfully exported Review tag {tag_name} to {output_file}")
                else:
                    self.logger.warning(f"Expected output file not found: {source_file}")
                    # Check if there are any CSV files generated in the review folder
                    csv_files = [f for f in os.listdir(review_folder) if f.endswith('.csv')]
                    self.logger.info(f"CSV files found in review folder: {csv_files}")
