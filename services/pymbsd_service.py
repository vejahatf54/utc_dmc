...
class PyMBSdService:
    ...
    def fetch_service_packages() -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if config_manager.get_pymbsd_use_test_mode():
                packages_path = config_manager.get_pymbsd_test_packages_path()
                if not packages_path:
                    raise Exception("PyMBSd test packages path is not configured in config.json")
            else:
                packages_path = config_manager.get_pymbsd_packages_path()
                if not packages_path:
                    raise Exception("PyMBSd packages path is not configured in config.json")
            
            if not os.path.exists(packages_path):
                if config_manager.get_pymbsd_use_test_mode():
                    # Create test directory and sample files
                    PyMBSdService._create_test_environment(packages_path)
                else:
                    raise Exception(f"PyMBSd packages path is not accessible: {packages_path}\n\nPlease ensure:\n1. The network path is accessible\n2. You have proper permissions\n3. The path exists on the server")
            
            services = []
            zip_files = [f for f in os.listdir(packages_path) if f.endswith('.zip')]
            ...
            for zip_file in zip_files:
                ...
                try:
                    service_info = PyMBSdService._extract_service_info(zip_path, package_name)
                    if service_info:
                        # Check current service status
                        service_info["status"] = PyMBSdService._get_service_status(service_info["service_name"])
    ...
    def update_service_statuses(service_data: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for service in service_data:
            ...
            updated_service["status"] = PyMBSdService._get_service_status(service["service_name"])
        ...
        try:
            ...
            for service in services:
                try:
                    ...
                    service_dir = os.path.join(installation_path, service["package_name"])
                    ...
                    service_files = PyMBSdService._find_service_files(service_dir)
                    if not service_files:
                        raise Exception("Service package missing necessary files")
                    
                    # Install service
                    PyMBSdService._install_service(service_dir, service_files["exe"])
                    
                    # Set auto mode if requested
                    if auto_mode:
                        PyMBSdService._set_service_auto_mode(service_dir, service_files["xml"])
    ...
    def _uninstall_service(service_name: str):
        ...
        try:
            ...
            if result.returncode != 0:
                logger.warning(f"Failed to delete service {service_name}: {result.stderr}")
    ...
    def _stop_service(service_name: str):
        ...
        try:
            ...
            if result.returncode != 0:
                logger.warning(f"Failed to stop service {service_name}: {result.stderr}")
    ...
    @staticmethod
    def _create_test_environment(test_path: str):
        """Create a test environment with sample service packages"""
        try:
            os.makedirs(test_path, exist_ok=True)
            
            # Create sample test packages
            sample_services = [
                {"name": "PyMBSd_TestService1", "service_name": "TestService1"},
                {"name": "PyMBSd_TestService2", "service_name": "TestService2"},
                {"name": "PyMBSd_TestService3", "service_name": "TestService3"}
            ]
            
            for service in sample_services:
                zip_path = os.path.join(test_path, f"{service['name']}.zip")
                
                # Create a simple zip file with XML config
                with zipfile.ZipFile(zip_path, 'w') as zip_file:
                    # Create XML config content
                    xml_content = f"""<?xml version="1.0" encoding="UTF-8"?>
<service>
    <name>{service['service_name']}</name>
    <description>Test PyMBSd Service</description>
    <executable>pymbsd_{service['service_name'].lower()}.exe</executable>
    <startmode>Manual</startmode>
</service>"""
                    
                    # Add XML file to zip
                    xml_filename = f"{service['name']}/pymbsd_{service['service_name'].lower()}.xml"
                    zip_file.writestr(xml_filename, xml_content)
                    
                    # Add dummy executable info
                    exe_filename = f"{service['name']}/pymbsd_{service['service_name'].lower()}.exe"
                    zip_file.writestr(exe_filename, "dummy executable content")
            
            logger.info(f"Created test environment at {test_path} with {len(sample_services)} sample services")
            
        except Exception as e:
            logger.error(f"Error creating test environment: {e}")
            raise Exception(f"Failed to create test environment: {str(e)}")
    ...
    def _force_delete_directory(directory_path: str):
        ...
        try:
            ...
            logger.error(f"Error force deleting directory {directory_path}: {e}")
            raise