...
class ReviewCsvService:
    ...
    def _write_review_to_csv(self, review_file: Path):
        ...
        try:
            ...
            while proc.poll() is None:
                if self._cancel_event.is_set():
                    logger.info("Cancellation requested, terminating %s", review_file)
            ...
            if proc.returncode != 0:
                ...
                raise RuntimeError(f"dreview.exe failed for {review_file}:\n{stderr.decode()}")
    ...
    def fetch_review_file_data(self):
        ...
        try:
            ...
            logger.error("Error during fetch_review_file_data: %s", ex, exc_info=True)
            raise
        finally:
    ...
    def _cleanup_partial_files(self):
        ...
        for csv_file in self.folder_path.glob("*.csv"):
            ...
            try:
                ...
                logger.warning("Could not remove partial file %s: %s", csv_file, ex)
    ...
    def clean_csv_files(self):
        ...
        for csv_file in self.folder_path.glob("*.csv"):
            ...
            try:
                ...
                logger.warning("Could not clean CSV file %s: %s", csv_file.name, e)
    ...
    def merge_csv_files(self):
        ...
        for csv_file in csv_files:
            if csv_file.name != self.merged_file:  # Don't delete the merged file
                try:
                    csv_file.unlink()
                    logger.info("Removed individual CSV file: %s", csv_file.name)
                except Exception as e:
                    logger.warning("Could not remove CSV file %s: %s", csv_file.name, e)
    ...
    def run(self):
        ...
        try:
            ...
            logger.error("Error during review processing workflow: %s", ex, exc_info=True)
